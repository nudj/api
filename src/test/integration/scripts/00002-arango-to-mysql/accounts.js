  ...
  describe('for accounts table', () => {
    ...
    describe('with a full data set', () => {
      ...
      it('should transfer all scalar properties', async () => {
        ...
        expect(personRoles[0]).to.have.property('email', 'email1@domain.com')
        expect(personRoles[0]).to.have.property('emailAddresses', '["email1@domain.com","email2@domain.com"]')
        expect(personRoles[0]).to.have.property('data', '{"key":true}')
        expect(personRoles[0]).to.have.property('type', ENUMS.ACCOUNT_TYPES.GOOGLE)
      })
