...
xdescribe('00004 Enrich Client Companies', () => {
  before(async () => {
    await Promise.all([
  ...
  it('enriches unenriched companies with `client` value `true`', async () => {
    await executeScript()
    const stubCalls = fetchStubCalls(clearbitStub)
    expect(stubCalls).to.deep.equal(['Company Two', 'Company Three'])
  })

  it('does not enrich companies with `client` value `false`', async () => {
    await executeScript()
    const stubCalls = fetchStubCalls(clearbitStub)
    expect(stubCalls).to.not.include('Company Six')
  })
          ...
          await populateCollections(db, [
            {
              ...
              data: times(100, () => {
                ...
                return { _key: data, name: data, client: true }
              })
            }
          ])
        })
